...
%{endfor ~}
    primary:
      hosts:
        primary1:
          ansible_host: ${azurerm_postgresql_flexible_server.postgresql_server.fqdn}
          private_ip: ${azurerm_postgresql_flexible_server.postgresql_server.fqdn}
EOT
}
...
resource "local_file" "postgresql" {
  ...
  content  = <<EOT
---
pg_superuser_override: ${azurerm_postgresql_flexible_server.postgresql_server.administrator_login}
pg_superuser_connect:
${azurerm_postgresql_flexible_server.postgresql_server.administrator_login}@${azurerm_postgresql_flexible_server.postgresql_server.name}
pg_superuser_password_override: ${azurerm_postgresql_flexible_server.postgresql_server.administrator_password}
pg_host: ${azurerm_postgresql_flexible_server.postgresql_server.name}
EOT
}
